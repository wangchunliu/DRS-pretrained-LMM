...
def evaluate(model, valid_loader, tokenizer, loss_fn, step, stage):
        ...
        for batch in valid_loader:
            ...
            if stage == 'bpt':
                ...
                tgt_mask = token_mask(
                    tgt, mask.sum(-1), 0.35,
                    tokenizer.mask_token_id)
                src = batch_process(src, tgt_mask,
...
def main():
    ...
    parser.add_argument(
        '-stage', default='bpt', type=str, help='training stage')
    parser.add_argument(
        '-max_lr', default=1e-4, type=float, help='max learning rate')
    parser.add_argument(
    ...
    if opt.stage != 'bpt':
        model_bpt = 'checkpoints/mlm_bpt.chkpt'
        model.load_state_dict(torch.load(model_bpt))
